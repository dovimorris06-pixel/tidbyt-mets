"""
Mets MLB Live Score App for Tidbyt - Enhanced Edition
Full pre-game info, live scoring, and detailed post-game summary.
Includes balls/strikes and full 9-man lineup.
"""
...
METS_TEAM_ID = 121
MLB_SCHEDULE_URL = "https://statsapi.mlb.com/api/v1/schedule?sportId=1&teamId=%d&date=%s&hydrate=team,linescore,flags,liveLookin,person,probablePitcher,lineups"
...
def fetch_todays_game():
    cached = cache.get(CACHE_GAME_ID)
    if cached != None:
        return cached
    today = get_today()
    url = MLB_SCHEDULE_URL % (METS_TEAM_ID, today)
    ...
    if game_pk == "None" or game_pk == "":
        return None
    cache.set(CACHE_GAME_ID, game_pk, ttl_seconds=3600)
    return game_pk
    ...
    return render.Root(
        ...
        child=render.Column(
            ...
            children=[
                ...
                render.Box(height=1),
                render.Text(content="Go Mets!", color=COLOR_ORANGE, font="CG-pixel-3x5-mono"),
                render.Box(height=2),
                render.Marquee(
                    width=64,
                    child=render.Text(content=lineup_text, color=COLOR_GRAY, font="CG-pixel-3x5-mono"),
                    offset_start=64, offset_end=0,
                ),
            ],
        ),
    )
...
def render_live(g):
    is_mets_home = g["home_abbr"] == "NYM"
    away_color   = COLOR_ORANGE if not is_mets_home else COLOR_WHITE
    home_color   = COLOR_ORANGE if is_mets_home     else COLOR_WHITE
    ...
    inning_row = render.Text(
        content="%s %s%s" % (arrow, str(g["inning"]), dly),
        color=COLOR_CYAN, font="CG-pixel-3x5-mono",
    )

    # Row 3: Outs + Bases + Balls/Strikes
    ...
    ob_row = render.Row(children=[
        ...
        # Base dots
        make_dot(g["base1"] == 1, COLOR_ORANGE),
        render.Box(width=1, height=3, color=COLOR_BLACK),
        make_dot(g["base2"] == 1, COLOR_ORANGE),
        render.Box(width=1, height=3, color=COLOR_BLACK),
        make_dot(g["base3"] == 1, COLOR_ORANGE),
        render.Box(width=3, height=3, color=COLOR_BLACK),
        # Balls (green dots)
    ...
    # Row 4: Pitcher / Batter
    matchup_row = render.Text(
        content="P:%s B:%s" % (truncate(g["pitcher"], 7), truncate(g["batter"], 7)),
        color=COLOR_GRAY, font="CG-pixel-3x5-mono",
    )

    # Row 5: Last play scrolling
    last_play = g["last_play"] if g["last_play"] != "" else "Waiting..."
    scroll_row = render.Marquee(
        width=64,
        child=render.Text(content="Last: " + last_play, color=COLOR_WHITE, font="CG-pixel-3x5-mono"),
        offset_start=64, offset_end=0,
    )
...
def render_final(g):
    ...
    home_color   = COLOR_ORANGE if is_mets_home     else COLOR_WHITE
    away_color   = COLOR_ORANGE if not is_mets_home else COLOR_WHITE
    mets_won     = (is_mets_home     and g["home_score"] > g["away_score"]) or \
                   (not is_mets_home and g["away_score"] > g["home_score"])
    result_color = COLOR_GREEN if mets_won else COLOR_RED
    result_word  = "W" if mets_won else "L"
    ...
    away_rhe  = render.Text(
        content="%s %s  %s  %s" % (pad_right(g["away_abbr"],3), pad_right(str(g["away_score"]),2), pad_right(str(g["away_hits"]),2), str(g["away_err"])),
        color=away_color, font="CG-pixel-3x5-mono",
    )
    home_rhe = render.Text(
        content="%s %s  %s  %s" % (pad_right(g["home_abbr"],3), pad_right(str(g["home_score"]),2), pad_right(str(g["home_hits"]),2), str(g["home_err"])),
        color=home_color, font="CG-pixel-3x5-mono",
    )
    ...
    scroll_row = render.Marquee(
        width=64,
        child=render.Text(content="   |   ".join(parts), color=COLOR_YELLOW, font="CG-pixel-3x5-mono"),
        offset_start=64, offset_end=0,
    )
...
def render_no_game():
    return render.Root(
        child=render.Column(
            ...
            children=[
                render.Text(content="Mets", color=COLOR_ORANGE, font="6x13"),
                render.Box(height=2),
...
def render_loading(msg="Loading..."):
    return render.Root(
        child=render.Column(
            expanded=True, main_align="center", cross_align="center",
            children=[
                render.Text(content="NYM", color=COLOR_ORANGE, font="6x13"),
                render.Box(height=2),
                render.Text(content=msg, color=COLOR_GRAY, font="CG-pixel-3x5-mono"),
            ],
        ),
    )
...
def main(config):
    game_pk = fetch_todays_game()
    if game_pk == None:
        return render_no_game()
    data = fetch_live(game_pk)
    if data == None:
        return render_loading("No data")
    box = fetch_boxscore(game_pk)
    g = parse_all(data, box)
    status = g["status_code"]
    if status == "P":
        return render_pregame(g)
    elif status == "F":
        return render_final(g)
    else:
        return render_live(g)
